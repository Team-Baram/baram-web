...
export default function PreferenceForm({ id, preference }: PreferenceFormProps) {
  ...
  const save = () => {
    if (selectedPreference.key === 'activityType') {
      const isSelectedRunning = selectedOption.value === 'running'
      const opts = isSelectedRunning ? runningOptions : cyclingOptions
      updatePreferenceMutation.mutate({
        id,
        activityType: selectedOption.value as 'running' | 'cycling',
        distance: opts.distanceOptions[0].value,
        pace: isSelectedRunning ? opts.paceOptions[opts.paceOptions.length - 1].value : opts.paceOptions[0].value,
        activityDaysPerWeek: opts.activityDaysPerWeekOptions[0].value,
      })
    } else {
  ...
  return (
    <>
      ...
      <SelectDialogButton
        ...
        onClick={() => handleDialog('선호 거리 (km)', 'distance', preferenceOptions.distanceOptions)}
      />
      <SelectDialogButton
        label={`평균 페이스 ${isRunning ? '(분/km)' : '(km/h)'}`}
        value={preferenceOptions.paceOptions.find((option) => option.value === pace)?.label ?? ''}
        onClick={() =>
          handleDialog(`평균 페이스 ${isRunning ? '(분/km)' : '(km/h)'}`, 'pace', preferenceOptions.paceOptions)
        }
      />
      <SelectDialogButton
        label={`주간 ${isRunning ? '러닝' : '라이딩'} 횟수`}
        value={preferenceOptions.activityDaysPerWeekOptions.find((option) => option.value === activityDaysPerWeek)?.label ?? ''}
        onClick={() =>
          handleDialog(`주간 ${isRunning ? '러닝' : '라이딩'} 횟수`, 'activityDaysPerWeek', preferenceOptions.activityDaysPerWeekOptions)
        }
      />
      <CustomSaveDialog
